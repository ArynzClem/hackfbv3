...
for xd in range(10000):
	...
	g='AppleWebKit/537.36 (KHTML, like Gecko)Version/4.0 MQQBrowser/'
...
/_/   \_\_|   \__, |_| |_/___|\____|_|\___|_| |_| |_|
              |___/
			{m}•{k}•{h}•{sir} SCRIPT By : ArynzClem {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK You Acount NGAB !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\tWelcome[green]%s[white]To My Script '%(my_name)))
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Complete  <<[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Good Bye Thanks To My Sc{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
